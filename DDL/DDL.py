...
CONSTRAINT domain_user_flag CHECK (ID='ad_%' OR ID='as_%' OR ID='su_%'),
PRIMARY KEY(ID)
);
""")
tmp.append(
...
CONSTRAINT domain_task_threshold CHECK (TaskThreshold >=0 and TaskThreshold <= 100),
PRIMARY KEY(TaskID),
UNIQUE(TableName)
);
""")
...
CONSTRAINT domain_SubmitterID CHECK (SubmitterID='su_%'),
CONSTRAINT domain_AssessorID CHECK (AssessorID = 'as_%')
);
""")
